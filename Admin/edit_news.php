...
$id=$_REQUEST['id'];
if(is_numeric($id)){
    $news = mysqli_fetch_array(mysqli_query($conectDB, "SELECT * FROM news WHERE id='".$id."'"));
}else{
    echo "";
}
                ...
                <div class="row">
                    <div class="col-xl-6">
                        <div class="card custom-card card-body">
                            <div class="input-group">
                                ...
                                <input id="title" type="text" class="form-control" value="<?php echo $news['title']; ?>" placeholder="عنوان الخبر">
                            </div>
                            
                            <div class="input-group">
                                <div class="input-group-text text-muted"> <i class="ri-calendar-line"></i> </div>
                                <input type="text" class="form-control" id="date" value="<?php echo $news['date']; ?>" placeholder="تاريخ النشر">
                            </div>
                            
                            <div class="input-group">
                                <div class="form-check" style="width:100px; padding:10px; margin-right:20px">
                                    <input class="form-check-input" type="radio" name="status" value="1" id="status" <?php if( $news['status'] == 1) { echo 'checked'; } ?> />
                                    <label class="form-check-label" for="Radio-md">
                                        نشر 
                                    </label>
                                </div>
                                <div class="form-check" style="padding:10px; margin-right:20px">
                                    <input class="form-check-input" type="radio" name="status" value="0" id="status" <?php if( $news['status'] == 0) { echo 'checked'; } ?> />
                                    <label class="form-check-label" for="Radio-md">
                                        مسودة
                                    </label>
                                </div>
                    ...
                    <div class="col-xl-6">
                        <div class="card custom-card">
                                <form id="imguploader" data-single="true" method="post" action="controller/upload_image.php?dir=../../images/news&filename=<?php echo $news['image']; ?>" class="dropzone">
                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            ...
                            <div class="float-end" style="text-align: center; padding:10px;">
                                <button id="PostDataBTM" class="btn btn-primary">تحديث الخبر</button>
                            </div>                            
                        </div>
                    </div>
                </div>
